...
export const FORME_TASK = [
  ...
  "   else input), its `### Requires` as an array of objects `{ facet, fan_in? }`",
  "   — `facet` is the requirement facet-name STRING (use the field name `facet`,",
  "   NEVER `name`); set `fan_in: true` when the contract deliberately asks for",
  "   MANY producers of one kind of truth,",
